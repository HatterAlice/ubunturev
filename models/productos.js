...
Producto.update = function(id, producto, result){
  conexion.query("UPDATE productos SET nombre=?,precio=?,stock=?,img=?,nomProv=? WHERE id = ?", [producto.nombre,producto.precio,producto.stock,producto.img,producto.nomProv, id], function (err, res) {
    if(err) {
      console.log("error: ", err);
      result(null, err);
    }else{
